...
class UF(list):
    ...
    def call(self, func_name, args):
        ...
        for arg in farg_num(func) != 1 and [args] or get_input(args):
            ret = func(*arg)
            if ret != None:
                print(ret)
    def help(self, msg=None):
        ...
        for f in self:
            ...
            print('%s(%s):\n\t%s'%(f.func_name, format_args(args.args, args.defaults or []), f.__doc__))
...
def time_filt(line):
    ...
    return re.sub('(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}).(\d+)', lambda m: str(str2ts(m.group(0))), line)
...
def split_by_size(group_size=4):
    ...
    for line in sys.stdin:
        print ' '.join(line[i:i+gsize] for i in range(0, len(line), gsize)),
@uf
def mark(match_file):
...
def replace(f):
    ...
    print re.sub('\w+', lambda m: kv.get(m.group(0), m.group(0)), sys.stdin.read())
...
def grep_src(x):
    ...
    pat = x
    if hex_str: pat = '%s\|0x%s\|0x%s'%(x, hex_str, hex_str.upper())
